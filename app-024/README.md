...
| 谜格校验 | 9 种常见谜格规则引擎（详见[下文](#谜格校验能力说明)）；结果分「通过 / 存疑 / 不通过」，存疑项说明原因；无格谜只做基础校验。**人工复核**：对存疑条目可判为通过或改判不通过（必填理由与署名），重算时保留人工结论并标出判定人与时间；与自动结论不一致的条目可单独筛出，人工判定可撤销回自动结果 |
...
自动校验的「存疑」条目（语义扣合无法自动判定）需懂谜的人拍板：编辑页校验面板可对已保存谜条**判为通过**或**改判不通过**，必须填写理由与署名；判定记录（谁、何时、为什么）随谜条持久化，「保存谜条 / 重新校验全部谜格」重算自动结论时人工结论保留，生效结论以人工为准。判定后若再改谜面 / 谜底 / 谜目 / 谜格，该判定自动失效并回退自动结论（记录保留，可撤销或重新判定）。谜库页校验筛选的「人工改判（与自动不一致）」单独列出人工与自动结论不一致的条目；列表徽标按生效结论显示，人工判定的条目带「人工」标记。
...
type Riddle = {
  ...
  tags: string[];
  check: {
    verdict: 'pass'|'suspect'|'fail'; reasons: string[]; checkedAt: number;  // 自动结论，保存/重算时刷新
    review?: { verdict: 'pass'|'fail'; reason: string; reviewer: string; at: number; basis: string } | null;  // 人工复核，重算时保留
  };
};
...
| 测试 | 覆盖 | 结果 |
|---|---|---|
| 单元测试 119 例 | 谜格校验 49 例（各格正例/误例/跨检查组合、生僻字、多音字）、人工复核 13 例（判定指纹/生效结论/失效回退/重算保留/撤销/不一致筛选）、重复检测（不同标点判重/繁简归一/阈值以下不误报）、CSV 解析导出与导入预览、打印版式（6/9/12 条/页、超版告警）、**性能（2000 条筛选 <100ms）**、离线数据包集成 | 全部通过 |
