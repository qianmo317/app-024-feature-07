...
| 谜格校验 | 9 种常见谜格规则引擎（详见[下文](#谜格校验能力说明)）；结果分「通过 / 存疑 / 不通过」，存疑项说明原因；无格谜只做基础校验；**人工复核**：存疑条目可判为通过或改判不通过（必填理由与署名），重算不覆盖、可撤销 |
...
语义扣合判不了的「存疑」条目，可由懂谜的人在谜条编辑页**判为通过（放行）或改判不通过**，必须填写理由与署名：

- 人工结论优先于自动结论展示，记录判定人、时间与理由；保存与「重新校验全部谜格」都**不覆盖**人工结论
- 人工结论与自动结果不一致的条目，在谜库列表顶部单独提示，可用校验筛选「人工与自动不一致」单独列出
- 人工结论可撤销，撤销后回到自动校验结果
- 修改谜面 / 谜底 / 谜目 / 谜格后保存，原人工结论失效（复核针对的是当时的内容），需重新复核
...
type Riddle = {
  ...
  review?: { verdict: 'pass'|'fail'; reason: string; reviewer: string; at: number; autoVerdict: Verdict }; // 人工复核
};
...
| 测试 | 覆盖 | 结果 |
|---|---|---|
| 单元测试 118 例 | 谜格校验 49 例（各格正例/误例/跨检查组合、生僻字、多音字）、人工复核 12 例（判定留痕/必填校验/重算保留/内容修改失效/撤销/筛选）、重复检测（不同标点判重/繁简归一/阈值以下不误报）、CSV 解析导出与导入预览、打印版式（6/9/12 条/页、超版告警）、**性能（2000 条筛选 <100ms）**、离线数据包集成 | 全部通过 |
| E2E 20 例 | 导入→校验→出条→登记→导出全流程、搜索筛选、批量出条打印预览（双联/裁切线/14pt 谜面）、现场登记与重复登记提示、大屏分级提示、兑奖号码、CSV 导出 BOM 字节断言、设置持久化、**人工复核全流程（判为通过→重算保留→单独列出→撤销回自动）**、**300 张谜条 50 页无错位**、**离线登记落库不丢**、控制台零报错、错误路由容错 | 全部通过 |
...
│   │   ├── review.ts           # 人工复核：有效结论 / 人工与自动不一致判定
